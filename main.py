        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(f"{PAYPAL_BASE_URL}/v1/oauth2/token", headers=headers, data=data) as response:
                if response.status == 200:
                    ...
                    print(f"❌ PayPal auth failed: {response.status}")
                    return None
    except Exception as e:
            ...
            "purchase_units": [
                {
                    ...
                    "description": "2 AI Video Credits"
                }
            ],
            "application_context": {
                "return_url": f"{os.getenv('BASE_URL', 'http://localhost:8000')}/api/paypal/success",
                "cancel_url": f"{os.getenv('BASE_URL', 'http://localhost:8000')}/api/paypal/cancel"
            }
        }
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(f"{PAYPAL_BASE_URL}/v2/checkout/orders", headers=headers, json=order_data) as response:
                if response.status == 201:
                    ...
                    print(f"❌ PayPal order creation failed: {response.status}")
                    text = await response.text()
                    print(f"❌ PayPal error: {text}")
                    return None
    except Exception as e:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(f"{PAYPAL_BASE_URL}/v2/checkout/orders/{order_id}/capture", headers=headers) as response:
                if response.status == 201:
                    ...
                    print(f"❌ PayPal capture failed: {response.status}")
                    text = await response.text()
                    print(f"❌ PayPal capture error: {text}")
                    return None
    except Exception as e:
...
async def process_uploaded_audio(audio_file: UploadFile, job_dir: Path) -> str:
    """Process uploaded audio - trim to 60 seconds max"""
    try:
        print(f"📁 Processing uploaded audio file: {audio_file.filename}")
        
        # Save uploaded file
        original_audio_path = job_dir / f"uploaded_original_{audio_file.filename}"
...
async def create_paypal_payment(current_user: dict = Depends(get_current_user)):
    """Create PayPal order for $1 (2 video credits)"""
    try:
...
async def capture_paypal_payment(request: Request, current_user: dict = Depends(get_current_user)):
    """Capture PayPal payment and add credits"""
    try:
        data = await request.json()
        ...
        if result.get('status') == 'COMPLETED':
            ...
            try:
                ...
                if user:
                    ...
                    print(f"💰 Payment successful! Added 2 credits to {user.email}. New balance: {user.video_credits}")
                    
                    return {
                        "message": "Payment successful! Added 2 video credits to your account.",
                        "new_balance": user.video_credits
                    }
                else:
...
if __name__ == "__main__":
    import uvicorn
    uvicorn.run(app, host="0.0.0.0", port=int(os.getenv("PORT", 8000)))# main.py - Complete AI Video Maker with PayPal Integration (NO STRIPE!)
from fastapi import FastAPI, HTTPException, Request, Depends, Form, File, UploadFile
from fastapi.responses import HTMLResponse, FileResponse, JSONResponse, RedirectResponse
from fastapi.middleware.cors import CORSMiddleware
from fastapi.staticfiles import StaticFiles
